...
def search_creator(page, creator_id):
 ...
 btn.click()
 print(f" ✅ 点击了搜索按钮")
 time.sleep(random.uniform(5, 10)) # 等待搜索弹窗出现
 ...
 # 2. 逐键输入（正确方法！）
 for char in creator_id:
 inp.press(char)
 time.sleep(0.1)

 time.sleep(0.5)
 print(f" ✅ 真正输入成功: @{creator_id}")
 ...
 print(f" ❌ 输入框定位失败: {input_sel}")
 print(f" 🔴 异常信息: {str(e)}") # 这里会打印真实错误
 continue
 else:
...
def click_search_result(page, creator_id):
 ...
 try:
 # ✅ 最稳：用 TikTok 官方给的唯一标识 data-e2e="search-card-user-link"
 user_link = page.locator(f'a[href="/@{creator_id}"]').first
...
def run_dm_task(tiktok_url, message):
 ...
 # 🔥 关键：获取内部真正的文字 <div>回关</div>
 btn_text = locator.locator('[data-testid="tux-web-text"]').text_content().strip()

 # ✅ 只在 关注 或 回关 时点击
 if btn_text == "关注" or btn_text == "回关":
 box = locator.bounding_box()
 if box:
